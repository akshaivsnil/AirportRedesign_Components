...
import com.akshai.dev.airportappredesign.clickable
import com.akshai.dev.airportappredesign.displayText
...
private val wayColor = Color(0xFF008489)
...
fun MonthlyCalenderView(
    ...
    val checkInDisplayDate = remember { mutableStateOf(LocalDate.now()) }
    val checkOutDisplayDate = remember { mutableStateOf<LocalDate?>(null) }
    val daysOfWeek = remember { daysOfWeek() }
    var selection by remember { mutableStateOf(DateSelection(startDate = today)) }
    val context = LocalContext.current


    Column(modifier = Modifier.padding(start = 13.dp , end = 13.dp, top = 0.dp)) {
        ...
        val checkOutView by remember { mutableStateOf(mViewModel.checkState.value.selection) }


        CalendarTitle(
            modifier = Modifier.padding(vertical = 0.dp, horizontal = 13.dp),
            ...
            dayContent = { value ->
                Day(
                    ...
                    if (day.position == DayPosition.MonthDate && (day.date == today || day.date.isAfter(
                        ...
                        println("Akshai -> ${checkOutView.name} | ${mViewModel.checkState.value.selection} ")
                        if (day.date.isBefore(selection.startDate) &&
                            mViewModel.checkState.value.selection.name == CheckInOutSelection.Out.name) {
                            Toast.makeText(
                                context,
                                "Check-out date can\\'t be before Check-in date.",
                                Toast.LENGTH_SHORT
                            ).show()
                        } else if (day.date.isBefore(selection.startDate) &&
                            mViewModel.checkState.value.selection.name != CheckInOutSelection.Out.name) {
                            ...
                            setDateOnCheckSwitch(
                                mViewModel,
                                mViewModel.checkState.value.selection,
                                day.date.toString()
                            )

                        } else if (mViewModel.checkState.value.selection.name != CheckInOutSelection.Out.name && selection.endDate != null && day.date.isBefore(
                                selection.endDate
                            )
                        ) {
                            checkOutDate = selection.endDate
                            ...
                            if ( mViewModel.checkState.value.selection.name == CheckInOutSelection.Out.name) {
                        ...
                        } else if ( mViewModel.checkState.value.selection.name != CheckInOutSelection.Out.name && checkOutDate != null && day.date.isAfter(
                                checkOutDate
                            )
                        ) {
                            checkOutDate = null
...
fun setDateOnCheckSwitch(mViewModel: MViewModel, selection: CheckInOutSelection, date: String) {
    when (selection) {
        CheckInOutSelection.In -> mViewModel.onEvent(UIEvent.CheckInChanged(date))
        CheckInOutSelection.Out -> mViewModel.onEvent(UIEvent.CheckOutChanged(date))
    }
}
    ...
    Row(
        ...
        Text(
            modifier = Modifier
                .weight(1f)
                .testTag("MonthTitle"),
            text = currentMonth.displayText().toUpperCase(Locale.current),
            color = wayColor,
            fontSize = 14.sp,
            textAlign = TextAlign.Center,
            fontWeight = FontWeight.Medium,
